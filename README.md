...
عند تسجيل الدخول لأول مرة سيُطلب تفعيل المصادقة الثنائية عبر تطبيق مثل Google Authenticator.
بعد إتمام الخطوة يمكن إدارة المستخدمين عبر صفحة `/users` وتعديل أدوارهم بين `admin` و`editor`.
